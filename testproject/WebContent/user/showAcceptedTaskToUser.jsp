...
function giveFeedback(){
	...
	popUpWithServerData("<%=request.getContextPath()%>/userFeedbackInput.action?tasks.taskId="+$("#taskId").val(),"Feedback",800,800);
}
</script>
		<script type="text/javascript">
	...
	<body>
		<form name="taskForm">
			<jmesa:tableModel id="tag" items="${tasksList}" maxRows="8"
				...
				<jmesa:htmlTable width="100%">
					<jmesa:htmlRow>
						...
						<jmesa:htmlColumn property="location.area" title="Area" filterable="false"/>
						<jmesa:htmlColumn property="scheduleDate" pattern="dd/MM/yyyy" 
							title="Schedule Date"cellEditor="org.jmesa.view.editor.DateCellEditor" filterable="false"/>
			...
			--%><input type="button" onclick="return giveFeedback()" value="Give Feedback" />
